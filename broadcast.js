...
client.on("message", async message => { 
  ...
  if (command == "bc") {  // للونلاين و الاوفلاين
    ...
    message.channel
      .send(
        `**هل أنت متأكد من إرسالك البرودكاست؟nمحتوى البرودكاست: `${args}`**` // `${args}` اعرف ان في خطا هنا لو نقدر تحله حله لو ما نقدر كلمني خاص و لو ما قدرت احذف 
      )
      .then(m => {
  ...
  if (command == "bco") { //للونلاين فقط
    ...
    message.channel
      .send(
        `**هل أنت متأكد من إرسالك البرودكاست؟nمحتوى البرودكاست: `${args}`**`
      )
      .then(m => {
        m.react("✅").then(() => m.react("❌"));
